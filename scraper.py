...
def main():
    setup_db()
    print("HydNews University Scraper Starting...")
    universities = get_all_universities()
    if not universities:
        send_telegram("⚠️ Could not fetch university list")
        return
    send_telegram(f"✅ HydNews Started\nMonitoring {len(universities)} universities")
    cycle = 1
    while True:
        print(f"\n--- Cycle {cycle} — Checking {len(universities)} universities ---")
        for i, uni in enumerate(universities):
            print(f"[{i+1}/{len(universities)}] {uni['name']}")
            scrape_university(uni["name"], uni["id"])
            time.sleep(2)
        print(f"Cycle {cycle} complete. Waiting 5 minutes...")
        cycle += 1
        time.sleep(300)
